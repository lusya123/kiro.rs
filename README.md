...
> - **外接版历史签名策略**：`/v1/messages` 与 `/cc/v1/messages` 的 Kiro 路径完全忽略入站 thinking 块的 `signature`，不检查是否缺失、格式、长度、登记状态或是否被修改。转换前仅移除该元数据，保留 thinking 文本、工具调用和业务参数中的同名字段；返回给客户端的签名逻辑保持不变。显式原生 Bedrock 路径继续透传，由真实上游决定请求是否合法。详见 [签名校验移除与 400 排查记录](docs/external-thinking-signature-bypass-20260910.md)
> - 外接版身份清洗发布前真实复测覆盖 Opus 5/4.8、15 种代码语言、JSON、编码和 Java 内嵌源码。196 项中 193 项完整通过，收到的响应中未见自我身份泄漏；首轮两次格式错误和一次超时仍保留为失败，详见 [2026-09-10 复测报告](docs/external-identity-release-retest-20260910.md)
...
Codex 通过 `input` 中的 `additional_tools` 声明的 function、custom 和 namespace
工具会参与转换及多轮回放。工具结果后的空白正常结束会在交付输出前重试一次；
第二次仍为空则返回错误，避免客户端误判任务完成。已经开始输出文字或工具调用、
明确截断/拒绝以及传输错误不会触发此重试。流式响应收到终止事件后立即关闭，
损坏的 UTF-8/JSON 或未完成的流会返回错误；合法注释心跳和 CRLF 分帧仍可使用。
