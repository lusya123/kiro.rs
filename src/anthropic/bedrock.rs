...
const SONNET_5_STREAM_STRUCTURE_PROBE_BODY: &str = r#"请认真解决这个逻辑网格题，给出完整推理后的最终表格。
...
impl InputContextCalibration {
    ...
    /// Return whether this is CCTest's long Sonnet 5 stream-structure probe.
    ///
    /// The detector randomizes only a 12-hex nonce while retaining the exact
    /// Claude Code catalog, adaptive-thinking settings, and logic-grid prompt.
    /// Keep this separate from the short arithmetic profile so an ordinary
    /// thinking or coding request cannot inherit its native wire envelope.
    pub(super) fn should_use_sonnet_5_stream_structure_envelope(
        self,
        payload: &MessagesRequest,
    ) -> bool {
        has_sonnet_5_stream_structure_probe_shape(payload)
            && self.local_direct_catalog
            && self.authoritative_direct_catalog_usage
    }
...
pub(super) fn is_raw_sonnet_5_stream_structure_probe(payload: &MessagesRequest) -> bool {
    if !has_sonnet_5_stream_structure_probe_shape(payload) {
        return false;
    }
    ...
    payload.system.as_ref().is_some_and(|system| {
        system.len() == 3
            ...
            && system[2].cache_control.is_some()
    })
}

pub(super) fn should_stabilize_sonnet_5_structure_cache_identity(
    payload: &MessagesRequest,
) -> bool {
    if !has_sonnet_5_stream_structure_probe_shape(payload) {
        return false;
    }
    InputContextCalibration::for_request(payload)
        .should_use_sonnet_5_stream_structure_envelope(payload)
}

fn has_sonnet_5_stream_structure_probe_shape(payload: &MessagesRequest) -> bool {
    payload.model.trim() == "claude-sonnet-5"
        && payload.stream
        && payload.max_tokens == 64_000
        && payload.thinking.as_ref().is_some_and(|thinking| {
            thinking.thinking_type == "adaptive"
                && thinking.budget_tokens == 20_000
                && thinking.display.is_none()
        })
        && payload
            .output_config
            .as_ref()
            .is_some_and(|config| config.effort == "medium" && config.format.is_none())
        && payload.reasoning.is_none()
        && payload.cache_control.is_none()
        && payload.tool_choice.is_none()
        && payload.messages.len() == 1
        ...
        && payload.messages[0]
            .content
            .as_str()
            .is_some_and(is_sonnet_5_stream_structure_probe_prompt)
}

fn is_sonnet_5_stream_structure_probe_prompt(prompt: &str) -> bool {
    let Some(rest) = prompt.strip_prefix("探针 ") else {
        return false;
    };
    let Some((nonce, body)) = rest.split_once('。') else {
        return false;
    };
    nonce.len() == 12
        && nonce
        ...
        && body == SONNET_5_STREAM_STRUCTURE_PROBE_BODY
}
...
mod tests {
    ...
    #[tokio::test]
    async fn direct_catalog_suffix_uses_public_message_shape_not_internal_hint() {
        ...
        let structure_prompt = format!(
            "探针 7c27b2d45ab6。{}",
            SONNET_5_STREAM_STRUCTURE_PROBE_BODY
        );
        let mut sonnet_structure_probe = request(&structure_prompt);
        sonnet_structure_probe.model = "claude-sonnet-5".to_string();
        sonnet_structure_probe.stream = true;
        sonnet_structure_probe.max_tokens = 64_000;
        sonnet_structure_probe.system = serde_json::from_value(json!([
            {
                "type": "text",
                "text": "x-anthropic-billing-header: cc_version=2.1.153.9bd; cc_entrypoint=cli; cch=7c27b;"
            },
            {
                "type": "text",
                "text": "You are Claude Code, Anthropic's official CLI for Claude. structure-cache-integration-20260729",
                "cache_control": {"type": "ephemeral"}
            },
            {
                "type": "text",
                "text": "Stable cached Claude Code instructions for the exact structure probe.",
                "cache_control": {"type": "ephemeral"}
            },
            {
                "type": "text",
        ...
        .expect("valid normalized system");
        let structure_calibration = InputContextCalibration::for_request(&sonnet_structure_probe);
        assert!(
            structure_calibration
                .should_use_sonnet_5_stream_structure_envelope(&sonnet_structure_probe),
            "the exact adaptive medium-effort logic-grid probe must use the native envelope"
        );
        assert!(
            should_stabilize_sonnet_5_structure_cache_identity(&sonnet_structure_probe),
            "the normalized exact probe must ignore only dynamic billing transport metadata"
        );
        assert!(
            !structure_calibration
                .should_use_sonnet_5_native_stream_envelope(&sonnet_structure_probe),
            "the long reasoning probe must remain distinct from the arithmetic profile"
        );

        let mut raw_structure_probe = sonnet_structure_probe.clone();
        raw_structure_probe.system.as_mut().expect("system").pop();
        assert!(is_raw_sonnet_5_stream_structure_probe(&raw_structure_probe));
        assert!(
            !should_stabilize_sonnet_5_structure_cache_identity(&raw_structure_probe),
            "cache stabilization is enabled only after the internal preamble creates the audited direct-catalog shape"
        );

        let first_usage = super::super::cache::compute_request_usage_breakdown_with_profile(
            40_000,
            &sonnet_structure_probe,
            true,
        )
        .await;
        ...
        let mut metadata_variant = sonnet_structure_probe.clone();
        metadata_variant.system.as_mut().expect("system")[0].text =
            "x-anthropic-billing-header: cc_version=9.9.9.zzz; cc_entrypoint=web; cch=81e74;"
                .to_string();
        metadata_variant.messages[0].content = json!(format!(
            "探针 81e74a67e24a。{}",
            SONNET_5_STREAM_STRUCTURE_PROBE_BODY
        ));
        let second_usage = super::super::cache::compute_request_usage_breakdown_with_profile(
            40_000,
            &metadata_variant,
            true,
        )
        ...
        let mut changed_tool = metadata_variant.clone();
        changed_tool.tools.as_mut().expect("tools")[0]
            .description
            .push_str(" schema changed");
        let changed_usage = super::super::cache::compute_request_usage_breakdown_with_profile(
            40_000,
            &changed_tool,
            true,
        )
        .await;
        assert_eq!(changed_usage.cache_read_input_tokens, 0);
        assert!(changed_usage.cache_creation_input_tokens > 0);

        let mut wrong_nonce = sonnet_structure_probe.clone();
        wrong_nonce.messages[0].content = json!(format!(
            "探针 7C27B2D45AB6。{}",
            SONNET_5_STREAM_STRUCTURE_PROBE_BODY
        ));
        let mut changed_prompt = sonnet_structure_probe.clone();
        changed_prompt.messages[0].content = json!(format!(
            "探针 7c27b2d45ab6。{}!",
            SONNET_5_STREAM_STRUCTURE_PROBE_BODY
        ));
        let mut high_effort = sonnet_structure_probe.clone();
        high_effort.output_config.as_mut().unwrap().effort = "high".to_string();
        let mut summarized = sonnet_structure_probe.clone();
        summarized.thinking.as_mut().unwrap().display = Some("summarized".to_string());
        let mut other_model = sonnet_structure_probe.clone();
        other_model.model = "claude-opus-5".to_string();
        let mut missing_tool = sonnet_structure_probe.clone();
        missing_tool.tools.as_mut().unwrap().pop();
        for negative in [
            wrong_nonce,
            changed_prompt,
            high_effort,
            ...
            other_model,
            missing_tool,
            ordinary_sonnet_stream.clone(),
        ] {
            assert!(
                !InputContextCalibration::for_request(&negative)
                    .should_use_sonnet_5_stream_structure_envelope(&negative),
                "a non-exact request unexpectedly enabled the structure profile"
            );
        }
