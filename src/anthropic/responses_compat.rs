        ...
        Some(Value::Object(text)) => {
            ...
            if let Some(format) = text.get("format") {
                if !format.is_null() {
                    ...
                    // Codex currently sends `strict` on the plain-text format. There is no
                    // schema to enforce for `type: text`, so accepting either boolean is
                    // semantically equivalent to the format without this hint. JSON-schema
                    // formats remain rejected above instead of silently weakening them.
                }
            }
        }
...
fn parse_function_tool(
    ...
    validate_strict_and_deferred_fields(object, path)?;
...
fn tool_catalog(request: &Value) -> Result<ToolCatalog, String> {
    let mut parsed = Vec::new();
...
fn append_input_item(
    ...
    match item_type {
        ...
        "reasoning" => {
            ...
            // Responses clients replay opaque reasoning items when `store: false`.
            // Kiro's upstream transport cannot consume OpenAI encrypted reasoning, and
            // treating summaries as assistant text would leak or distort hidden state.
            // Validate and intentionally omit the opaque item while retaining the visible
            // messages and tool results that follow it.
        }
        "additional_tools" => {}
...
mod tests {
    ...
    #[test]
    fn replayed_reasoning_is_validated_and_omitted_from_kiro_messages() {
        let request = json!({
            "model": "gpt-5.6-sol",
            "input": [
                {
