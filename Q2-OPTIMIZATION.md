...
## Iteration 4: bounded retry of context-only EOF (2026-09-06)

The three newly supplied Google PRO+ credentials are enabled on the isolated Q2 test instance; the quota block described above is historical. The old exhausted credentials remain disabled. All seven models were verified before this iteration. No credentials are stored in this document or the repository.

A replay of CheckHub's original 382,013-byte Opus 5 cache request produced an HTTP 200 with no answer. The upstream capture contains one CRC-valid 148-byte `contextUsageEvent` (15.326799392700195% context usage), followed by clean EOF and no model/terminal event. Production code now uses at most 16 KiB / one second of initial EventStream lookahead and retries this narrowly recognized condition once with identical request bytes. Model, tool, thinking/signature, whitespace, refusal, terminal, error, unknown/malformed and full-context events pass through. Two such empty attempts produce an explicit 502. Request conversion, headers, parameters, prompts, token accounting, `msg_bdrk_` IDs and thinking signatures are unchanged. A same-byte live retry recovered a real answer; a separate two-empty-attempt sample correctly returned 502. Later EOF beyond the lookahead window remains a limitation.

Runtime commit: `3d2912d684501a4166c73d2c38a22156bcead71c`. Test-only image: `ghcr.io/lusya123/kiro-rs:q2-test-3d2912d684501a4166c73d2c38a22156bcead71c`, digest `sha256:bd5b6d22d6d834ce6c9ffd0a35edd3b8d5fbb1e6edb7a210fd51870fe4589e29`. CI: https://github.com/lusya123/kiro.rs/actions/runs/33986484554 . Q2 now routes to `kiro-rs-q2-retry-3d2912d`, `127.0.0.1:19008`, with config `/home/ubuntu/services/q2-rollouts/20260906-3d2912d-context-only/config`. Existing 44 container fingerprints were unchanged. The old 19007 container remains running with all credentials disabled; rollback requires transferring credentials with no overlap before switching Nginx, not just changing the port.

Validation: 986 tests passed, zero failed, three ignored. Candidate and public-domain ordinary workloads each returned 28 HTTP 200 responses, with 27/28 strict assertions: one ticket value was a JSON string instead of integer, as already observed in the prior iteration. All values were preserved, and no output rewriting was added. Seven generated functions passed actual execution examples in both rounds; seven additional streaming code requests had complete start/stop events and code.

All 20 site-supported combinations completed; eight unsupported model-selector cells remain explicit. CheckHub Opus 5 changed 85→93; Opus 4.8 stayed 91 and 4.7 stayed 85. All 18 large-cache calls returned text, but Opus 5 was prewarmed by exact-body replays, so its score increase is not causal proof of the fix or a cold-cache guarantee. 9coding: Opus 4.8 88.6→87.3, Opus 4.7 81→82.3, Sonnet 4.6 80.4→79.1; other scores held. Dance Sonnet 5 first scored 5/8 (baseline output 59 below the site's 64 threshold), then 6/8 on an unchanged repeat; retain both, with 5/8 primary. Other Dance scores held. Ranking page scores remain 100, with observed internal Sonnet 5/4.6 scores 88/88; Opus 5 internal score was not retained.

Artifacts: `outputs/q2-cache-reliability-20260906/` in the main checkout. The ingress archive has 686 requests, 685 complete responses, and one explicitly missing original Dance Opus 5 STREAM-OK response. A pre-existing keepalive connection caused an initial ordinal pairing shift; 17 affected records were audited, with 16 recovered using connection chronology (one from the matching client workload record), and one missing. Raw packets were already removed when discovered; do not claim complete wire coverage. The revised capture guard rejects count mismatches and response-before-request chronology before cleanup. The initial browser memory export also failed to persist: seven 9coding history pages, two Ranking reports, 24 CheckHub dialogs and eight official Dance downloads were recovered; Opus 5 Ranking/CheckHub headline results are observed summaries with server request/response evidence, not retained original UI snapshots. Full limitations and all first/repeat scores are documented in the report. No new POMO baseline was run in this iteration.
